...
class AzureBlobManager:
    ...
    def load_object(self, file_name: str) -> Any:
        ...
        try:
            ...
            if file_format == "csv":
                ...
                logger.info(
                    f"Successfully loaded {file_format.upper()} file {file_name}"
                )
            else:
    ...
    def write_object(self, file_name: str, data: Any):
        """
        Writes an object to Azure Blob Storage in the appropriate file format.
        """
        try:
            _, file_extension = os.path.splitext(file_name)
            file_format = file_extension.lstrip(".").lower()
            ...
            if file_format in ["csv", "parquet", "json", "pickle", "txt"]:
                if file_format == "csv":
                    output = io.StringIO()
                ...
                logger.info(
                    f"Successfully wrote {file_format.upper()} file: {file_name}"
                )

            elif file_format in ["pcm", "mp3", "wav", "flac", "aac"]:
                if not isinstance(data, bytes):
                    logger.error("Data for audio files must be in byte format.")
                    raise TypeError("Data for audio files must be in byte format.")

                content = data
                blob_client.upload_blob(content, blob_type="BlockBlob", overwrite=True)
                logger.info(f"Successfully wrote audio file: {file_name}")

            else:
                logger.error(f"Unsupported file format for writing: {file_format}")
                raise ValueError(f"Unsupported file format for writing: {file_format}")

        except Exception as e:
            logger.error(f"Error writing object to blob: {e}")
            raise
